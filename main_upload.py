...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://happepls.pics",  # 替换为您的域名
        "https://happepls.pics", # 如果将来要用 HTTPS
        "http://localhost:9000",
        "http://127.0.0.1:9000"
    ],
...
class AudioInput(BaseModel):
    ...
    locales: str = Field('[]', description="JSON array of locales to detect")
...
# 新增 JSON 格式端点
@app.post("/transcribe-json", summary="Transcribe Audio from URL (JSON format)")
async def transcribe_json(input_data: AudioInput):
    """
    专门为 JSON 格式请求创建的转录端点,也是快速转录,没办法coze暂不支持multipart/form-data格式
    """
    try:
        # 验证必要的环境变量
        ...
        data.add_field('definition', f'{{"locales": {input_data.locales}}}')
...
async def real_time_transcription(input_data: AudioInput):
    ...
    try:
        ...
        # 处理本地 URL，直接读取文件
        if input_data.audio_url.startswith("http://localhost:9000/audio/"):
